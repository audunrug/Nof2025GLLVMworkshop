...
So, $\Sigma$ is a also a measure of how much the community changes from one site to another. Two sites with strong negative correlation are predicted to have different communities, and the same community with strong positive correlation. Thus, we can \textbf{predict} turnover.
...
```{r, test}
library(gllvm)
model <- gllvm(y, X = X, formula = ~(0+NO3|1) + (Water_Type|1), beta0com = TRUE, family = "negative.binomial", num.lv = 0)
```
